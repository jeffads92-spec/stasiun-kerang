...
function handleLogin($pdo) {
    ...
    // Rate limiting: maksimal 5 percobaan dalam 15 menit
    if (!isset($_SESSION['login_attempts'])) {
        $_SESSION['login_attempts'] = 0;
        $_SESSION['first_failed_attempt'] = time();
    }
    
    $timeWindow = 900; // 15 menit dalam detik
    if ($_SESSION['login_attempts'] >= 5 && (time() - $_SESSION['first_failed_attempt']) < $timeWindow) {
        sendResponse(429, false, 'Terlalu banyak percobaan login. Coba lagi nanti.');
        return;
    }
